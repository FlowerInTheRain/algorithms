...
object BuzzleObj {
    ...
    @tailrec
    private def recursiveSumOfDigits(
        num: Int,
        input: String,
        validator: Int,
        base: Int): Boolean = {
        if (input.length <= 1) {
            ...
            val res: Int = input
                .split("")
                .map(x => convertFromAnyBaseToDecimal(x, base))
                .sum
            if (
              res % validator == 0 || convertFromAnyBaseToDecimal(
                Integer.toString(res, base).last.toString,
                base
              ) == validator
            ) {
                true
    ...
    private def checkNumber(
        ...
        inputs
            .map(modulo => {
                ...
                convertFromAnyBaseToDecimal(
                  Integer.toString(num.toInt, base).last.toString,
                  base
                ) == modulo.toInt ||
                recursiveSumOfDigits(
                  num.toInt,
                  Integer.toString(num.toInt, base),
                  modulo.toInt,
                  base
                )
            })
